...
import { isLegacyPair, markDocked, toOrderTuple, type StoredStrategy } from "@/lib/strategy-store";
...
export function StrategyCard({ strategy, onChanged }: { strategy: StoredStrategy; onChanged: () => void }) {
  ...
  // A reverting quote is an answer, not a gap: reading only `.result` showed
  // a side that can never fill as a plain em-dash, with its button still live.
